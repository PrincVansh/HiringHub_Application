...
public class ResultsDAO {
    ...
    public static List<ParticipantResultPojo> getResultedParticipants(String hrid)throws SQLException
    {
        Connection conn=DBConnection.getConnection();
        PreparedStatement ps=conn.prepareCall("select name,userid,jobs.jobid,jobtitle,percentage from users,jobs,results where results.jobid=jobs.jobid and results.participantid=users.id and jobs.hrid=?");
        ps.setString(1,hrid);
        ResultSet rs=ps.executeQuery();
        List<ParticipantResultPojo>list=new ArrayList<>();
        while(rs.next())
        {
    ...
    public static boolean changeStatus(String id)throws SQLException
    {
        Connection conn=DBConnection.getConnection();
        PreparedStatement ps=conn.prepareStatement("update results set selectedbyhr='yes' where participantid=?");
        ps.setString(1, id);
